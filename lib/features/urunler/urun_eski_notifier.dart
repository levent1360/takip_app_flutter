...
final urunEskiNotifierProvider =
    StateNotifierProvider<UrunEskiNotifier, UrunState>((ref) {
      return UrunEskiNotifier(ref);
    });

class UrunEskiNotifier extends StateNotifier<UrunState> {
  final Ref ref;

  UrunEskiNotifier(this.ref) : super(UrunState.initial()) {}
  ...
  Future<void> initData({bool isClearAll = false}) async {
    ...
    final apiResponse = await urunController.getUrunlerPageSearch(1);
  ...
  Future<void> nextData({bool isClearAll = false}) async {
    ...
    final apiResponse = await urunController.getUrunlerPageSearch(nextPage);
    await urunController.urunGoruldu();
